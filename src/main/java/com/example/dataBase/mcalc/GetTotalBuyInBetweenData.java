...
/**
 * Class to retrieve Total Buy/Sell transaction data from SEC_ACC_TRNXS table
 * Contains methods to calculate sum of amounts for transactions between dates
 */
public class GetTotalBuyInBetweenData {

    private static final Logger logger = Logger.getLogger(GetTotalBuyInBetweenData.class);
     ...
     */
    public GetTotalBuyInBetweenData(OracleDBConnection dbConnection) {
        this.dbConnection = dbConnection;
    }

    /**
     * Get total BUY amount for NIN, Company, and Date Range
     ...
     * @return Total buy amount, or null if no data found
     */
    public Double getTotalBuyByNinAndCompanyBetweenDates(String nin, String startDate, String endDate, String companyCode) {
        try {
            logger.info("Calculating total BUY amount for NIN: " + nin + ", Company: " + companyCode + " between " + startDate + " and " + endDate);
            ...
            if (!results.isEmpty() && results.get(0).get("TOTAL_AMOUNT") != null) {
                ...
                logger.info("Total BUY amount calculated: " + totalAmount);
                return totalAmount;
            } else {
                logger.warn("No BUY transactions found for NIN: " + nin + ", Company: " + companyCode + " between dates");
                return null;
            }

        } catch (SQLException e) {
            logger.error("Error calculating total BUY amount: " + e.getMessage(), e);
            return null;
        }
    }
    ...
    /**
     * Get total BUY amount for NIN and Company (all dates)
     * @param nin NIN to query
     * @param companyCode Company code to query
     * @return Total buy amount, or null if no data found
     */
    public Double getTotalBuyByNinAndCompany(String nin, String companyCode) {
        try {
            logger.info("Calculating total BUY amount for NIN: " + nin + " and Company: " + companyCode);
            ...
            if (!results.isEmpty() && results.get(0).get("TOTAL_AMOUNT") != null) {
                ...
                logger.warn("No BUY transactions found for NIN: " + nin + " and Company: " + companyCode);
                return null;
            }
    ...
    /**
     * Get total amount for any transaction type (BUY, SELL, etc.)
     * @param nin NIN to query
     * @param startDate Start date in format 'DD-Mon-YYYY'
     * @param endDate End date in format 'DD-Mon-YYYY'
     * @param companyCode Company code to query
     * @param trnxType Transaction type (BUY, SELL, etc.)
    ...
    /**
     * Get total BUY amount for NIN across all companies between dates
     * @param nin NIN to query
     * @param startDate Start date in format 'DD-Mon-YYYY'
     * @param endDate End date in format 'DD-Mon-YYYY'
     * @return Total buy amount, or null if no data found
     */
    public Double getTotalBuyByNinBetweenDates(String nin, String startDate, String endDate) {
        try {
            logger.info("Calculating total BUY amount for NIN: " + nin + " between " + startDate + " and " + endDate);
            ...
            if (!results.isEmpty() && results.get(0).get("TOTAL_AMOUNT") != null) {
                ...
                logger.warn("No BUY transactions found for NIN: " + nin + " between dates");
                return null;
            }
