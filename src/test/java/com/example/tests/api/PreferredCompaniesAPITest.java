...
import com.example.api.PreferredCompaniesAPI;
...
/**
 * Test class for PreferredCompaniesAPI
 *
 * WHAT IS BEING TESTED:
 * - PREFERRED_COMPANIES API endpoint functionality
 ...
 * - SessionID: Read dynamically from api-config.properties
 * - Service: "PREFERRED_COMPANIES"
 * - Version: "1.0"
 * - AppId: "WEB_ORDERS.25"
 ...
 * - Response contains list of preferred companies
 * - Each company has: CompanyCode, CompanyName, IsPreferred flag
 * - Response is properly deserialized to PreferredCompaniesResponseDTO
 *
 * VALIDATION APPROACH:
 * 1. Assert response is not null
 * 2. Assert HTTP status code is 200
 * 3. Assert response body is not empty
 * 4. Assert response can be deserialized to DTO
 * 5. Assert companies list is not empty (if available)
 * 6. Validate each company has required fields
 * 7. Log all request/response details for debugging
 * 8. Attach evidence to Allure report
 */
@Epic("API Testing")
@Feature("Preferred Companies API")
public class PreferredCompaniesAPITest {

    private static final Logger logger = LoggerFactory.getLogger(PreferredCompaniesAPITest.class);
    private PreferredCompaniesAPI preferredCompaniesAPI;
    ...
    @BeforeClass
    @Step("Initialize PreferredCompaniesAPI client")
    public void setup() {
        System.out.println("\n" + "=".repeat(70));
        System.out.println("INITIALIZING PREFERREDCOMPANIESAPI TEST");
        System.out.println("=".repeat(70));

        preferredCompaniesAPI = new PreferredCompaniesAPI(BASE_URL);

        System.out.println("Base URL: " + BASE_URL);
        System.out.println("Endpoint URL: " + ENDPOINT_URL);
        System.out.println("Test NIN: " + TEST_NIN);
        System.out.println("PreferredCompaniesAPI client initialized successfully");
        System.out.println("=".repeat(70) + "\n");

        logger.info("PreferredCompaniesAPI Test Suite initialized");
    ...
    public void ensureSessionIDExists() {
        ...
        try {
            ...
            System.out.println("✓ PreferredCompaniesAPI tests can proceed");
            System.out.println("=".repeat(70) + "\n");
    ...
    @Test(priority = 1)
    @Story("Send PREFERRED_COMPANIES Request with RestAssured + DTO")
    @Description("Test sending PREFERRED_COMPANIES request using RestAssured with DTO objects and Jackson serialization")
    @Severity(SeverityLevel.CRITICAL)
    public void testGetPreferredCompanies() {
        System.out.println("\n" + "=".repeat(70));
        System.out.println("TEST: GET PREFERRED COMPANIES REQUEST");
        System.out.println("=".repeat(70));
        ...
        try {
            ...
            System.out.println("\nSTEP 2: PREPARING PREFERRED_COMPANIES REQUEST");
            System.out.println("-".repeat(70));

            System.out.println("Endpoint: " + ENDPOINT_URL);
            System.out.println("Service: PREFERRED_COMPANIES");
            ...
            Allure.step("Prepare PREFERRED_COMPANIES request");
            ...
            Allure.step("Send PREFERRED_COMPANIES request via RestAssured");
            List<PreferredCompaniesResponseDTO> responseList =
                preferredCompaniesAPI.getPreferredCompanies(ENDPOINT_URL, TEST_NIN);
            ...
            int httpStatusCode = preferredCompaniesAPI.getLastHttpStatusCode();
            ...
            Assert.assertEquals(httpStatusCode, 200,
                    "Expected HTTP status code 200 but got: " + httpStatusCode);

            System.out.println("✓ ASSERTION PASSED: HTTP Status Code is 200 (SUCCESS)");
            logger.info("✓ HTTP Status Code assertion PASSED: 200");
            ...
            PreferredCompaniesResponseDTO response = responseList.get(0);
            ...
            // ========== STEP 7: VALIDATE COMPANIES DATA ==========
            System.out.println("\nSTEP 7: VALIDATING COMPANIES DATA");
            System.out.println("-".repeat(70));

            Allure.step("Validate companies data in response");
            List<PreferredCompaniesResponseDTO.CompanyDTO> companies = response.extractCompanies();

            if (companies != null && !companies.isEmpty()) {
                System.out.println("✓ Companies list found");
                System.out.println("  Number of companies: " + companies.size());

                // Display first few companies
                int displayCount = Math.min(3, companies.size());
                System.out.println("  First " + displayCount + " companies:");
                for (int i = 0; i < displayCount; i++) {
                    PreferredCompaniesResponseDTO.CompanyDTO company = companies.get(i);
                    System.out.println("    " + (i + 1) + ". " + company.getCompanyCode() +
                                     " - " + company.getCompanyName() +
                                     " (Preferred: " + company.getIsPreferred() + ")");
                }

                // Validate company fields
                PreferredCompaniesResponseDTO.CompanyDTO firstCompany = companies.get(0);
                Assert.assertNotNull(firstCompany.getCompanyCode(), "Company code should not be null");
                Assert.assertNotNull(firstCompany.getCompanyName(), "Company name should not be null");

                System.out.println("✓ ASSERTION PASSED: Company data is valid");
                logger.info("✓ Companies data validated: {} companies found", companies.size());
            } else {
                System.out.println("⚠ No companies found in response (may be expected)");
                logger.warn("⚠ No companies found in response");
            }
            ...
            if (companies != null) {
                Allure.addAttachment("Companies Count", String.valueOf(companies.size()));
            }
            ...
            System.out.println("✓ Companies data attached to Allure report");
            ...
            System.out.println("✓ Step 7: Companies data validated");
            ...
            logger.info("╔════════════════════════════════════════════════════════════════╗");
            logger.info("║      TEST PASSED: PREFERRED_COMPANIES Request                 ║");
            logger.info("╚════════════════════════════════════════════════════════════════╝");
            ...
            logger.error("╔════════════════════════════════════════════════════════════════╗");
            logger.error("║      TEST FAILED: PREFERRED_COMPANIES Request                 ║");
            logger.error("╚════════════════════════════════════════════════════════════════╝");
    ...
    @Test(priority = 2)
    @Story("Send PREFERRED_COMPANIES Request with Custom DTO")
    @Description("Test sending request with manually constructed DTO including sessionId")
    @Severity(SeverityLevel.NORMAL)
    public void testGetPreferredCompaniesWithCustomDTO() {
        System.out.println("\n" + "=".repeat(70));
        System.out.println("TEST: PREFERRED COMPANIES WITH CUSTOM DTO");
        System.out.println("=".repeat(70));
        ...
        try {
            ...
            // Send request
            List<PreferredCompaniesResponseDTO> responseList =
                preferredCompaniesAPI.sendRequest(ENDPOINT_URL, request);
            ...
            System.out.println("✓ Response received: " + responseList.get(0).toString());
            System.out.println("=".repeat(70) + "\n");
    ...
    @Test(priority = 3)
    @Story("Get Single Preferred Companies Response")
    @Description("Test convenience method that returns single response object")
    @Severity(SeverityLevel.NORMAL)
    public void testGetPreferredCompaniesSingle() {
        System.out.println("\n" + "=".repeat(70));
        System.out.println("TEST: PREFERRED COMPANIES SINGLE RESPONSE");
        System.out.println("=".repeat(70));

        try {
            // Send request and get single response
            PreferredCompaniesResponseDTO response =
                preferredCompaniesAPI.getPreferredCompaniesSingle(ENDPOINT_URL, TEST_NIN);
